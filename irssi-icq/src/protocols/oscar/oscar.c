 ...
 * taken from: 
 * gaim
 * Modified by: Wilfried Goesgens <w.goesgens@chaosindustries.de>
 *
...
/// why? Where to put this best? don't overwrite irssi's config.h...
#include "irssi-icq-config.h"
...
static void gaim_icq_contactadd(struct name_data *data) {
	...
	if (g_list_find(gaim_connections_get_all(), gc)) {
	  /// -> irssify	show_add_buddy(gc, data->name, NULL, data->nick);
	}
...
static int gaim_parse_clientauto_ch4(aim_session_t *sess, char *who, fu16_t reason, fu32_t state, char *msg) {
	...
	switch(reason) {
		case 0x0003: { /* Reply from an ICQ status message request */
			...
			/// -> irssify g_show_info_text(gc, who, 2, dialog_msg, NULL);
...
static int gaim_parse_user_info(aim_session_t *sess, aim_frame_t *fr, ...) {
	...
	if (infotype == AIM_GETINFO_AWAYMESSAGE) {
		if (evilhack) {
/// -> irssify			g_show_info_text(gc, info->sn, 2,
/// -> irssify					 header,
/// -> irssify					 (utf8 && *utf8) ? away_subs(utf8, username) :
/// -> irssify					 _("<i>User has no away message</i>"), NULL);
		} else {
/// -> irssify			g_show_info_text(gc, info->sn, 0,
/// -> irssify					 header,
/// -> irssify					 (utf8 && *utf8) ? away_subs(utf8, username) : NULL,
/// -> irssify					 (utf8 && *utf8) ? "<hr>" : NULL,
/// -> irssify					 NULL);
		}
	} else if (infotype == AIM_GETINFO_CAPABILITIES) {
/// -> irssify		  g_show_info_text(gc, info->sn, 2,
/// -> irssify				  header,
/// -> irssify				  "<i>", _("Client Capabilities: "),
/// -> irssify				  caps_string(info->capabilities),
/// -> irssify				  "</i>",
/// -> irssify				  NULL);
	} else {
/// -> irssify		g_show_info_text(gc, info->sn, 1,
/// -> irssify				 (utf8 && *utf8) ? away_subs(utf8, username) : _("<i>No Information Provided</i>"),
/// -> irssify				 NULL);
	}
...
/// -> irssify	while (gtk_events_pending())
/// -> irssify		gtk_main_iteration();
...
static GList *oscar_actions(GaimConnection *gc)
{
	...
	/// -> irssify	pam->callback = show_set_info;
	pam->gc = gc;
	...
	/// -> irssify pam->callback = show_change_passwd;
	pam->gc = gc;
	m = g_list_append(m, pam);
...
/// -> irssify	pam->callback = show_find_email;
	pam->gc = gc;
	m = g_list_append(m, pam);
