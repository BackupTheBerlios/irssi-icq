...
void serv_finish_login(GaimConnection *gc)
{
  /*
	...
	if (gaim_account_get_user_info(account) != NULL) {
	  //// was buf = strdup_withhtml(gc->user->user_info);
...
static void des_popup(void *a, void* b)//GtkWidget *w, GtkWidget *window)
{/*
	if (w == window) {
...
static void
url_clicked_cb(/*GtkWidget *w,*/ void *a,  const char *uri)
{
